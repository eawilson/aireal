...
class HTMLAttr(MutableMapping):
    def __init__(self, *args, **kwargs):
        # args is a series of dictionaries
        super().__init__()
        self.attr = {}
...
class Fields(MutableMapping):
    ...
    def __setitem__(self, key, val):
        if not isinstance(val, Element):
            raise TypeError(f'"{key}" is not of type Element.')
    ...
    def __setattr__(self, attr, val):
        if isinstance(val, Element):
            if hasattr(self, attr):
                ...
                if previous is not None and not isinstance(previous, Element):
...
class Form(Fields):
    ...
    def validate(self, data=None):
        ...
        if "csrf" in session and self.csrf != session["csrf"]:
            return False
        return all(field.validate() for field in self.values() if hasattr(field, "validate"))
    
    
    @property
    def errors(self):
        return any(getattr(field, "errors") for field in self.values())
...
class Element(object):
    ...
    def __repr__(self):
        return "{}({})".format(type(self).__name__, ", ".join("{}={}".format(k, v) for k, v in self.attr.items())) 
